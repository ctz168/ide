...
一款轻量级好用的的移动端 Web IDE，专为 Termux/Ubuntu 环境设计。
...
内置代码格式化功能，支持多种语言和格式化工具。在调试面板的"格式化"标签页中，可以：
- 查看当前系统已安装的格式化工具
- 一键格式化当前打开的文件
- 批量格式化整个工作区（支持预览模式）
- 自动检测文件类型选择对应的格式化器

支持的格式化器包括：Black (Python)、Prettier (JS/TS/HTML/CSS/JSON/YAML/MD)、Clang-Format (C/C++)、Shfmt (Shell)、Gofmt (Go)、Rustfmt (Rust) 等。
...
│   ├── formatter.py       # 代码格式化：Black/Prettier/Clang-Format/Shfmt 等
...
| 方法 | 接口 | 说明 |
|------|------|------|
| GET | `/api/formatter/available` | 列出所有支持的格式化工具及其安装状态 |
| POST | `/api/formatter/format` | 格式化代码内容（内存中，不保存） |
| POST | `/api/formatter/format-file` | 格式化磁盘上的文件（原地修改） |
| POST | `/api/formatter/format-workspace` | 递归格式化整个工作区的文件 |
...
| `.js`, `.jsx`, `.ts`, `.tsx`, `.css`, `.scss`, `.less`, `.html`, `.htm`, `.json`, `.yaml`, `.yml`, `.md`, `.xml` | Prettier | `npm install -g prettier` |
| `.sh`, `.bash` | Shfmt | `go install mvdan.cc/sh/v3/cmd/shfmt@latest` |
| `.go` | Gofmt | (Go 自带) |
| `.rs` | Rustfmt | `rustup component add rustfmt` |
| `.sql` | sqlfmt | `go install github.com/jackc/sqlfmt/cmd/sqlfmt@latest` |
| `.c`, `.cpp`, `.h`, `.hpp` | Clang-Format | `apt install clang-format` / `brew install clang-format` |
| `.java` | Google Java Format | 从 GitHub 下载 |

**格式化工作区示例：**
```json
POST /api/formatter/format-workspace
{
  "path": "src",           // 可选，默认为工作区根目录
  "extensions": [".py", ".js"],  // 可选，默认所有支持的类型
  "dry_run": false         // true=仅预览，false=实际修改
}
```
